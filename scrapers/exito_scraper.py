...
def _extract_product_links_from_html(
    ...
    for link in _css(page, "a[href*='/p/']"):
        ...
        title = (
            _css(link, "::attr(title)").get()
            or _css(link, "::text").get()
            or ""
        ).strip()
...
def _extract_products_from_cards(
    ...
    for card in cards:
        ...
        # Extrae dentro de esa card el nombre, imagen, vendedor,
        # precio actual, precio original y descuento.
        name = (
...
def scrape_exito(
    ...
    for url in urls:
        ...
        for page in pages:
            ...
            # Estos son datos estructurados ocultos en el HTML que pueden
            # tener nombre, precio, URL e imagen.
            raw_json_blocks = _css(
                page,
